(() => {
  ...
  // demo/src/pcm_player.js
  var unlockAudioContext = (audioCtx) => {
  ...
  FUPCMPlayer.prototype.createContext = function() {
    ...
    this.audioCtx.onstatechange = () => console.log("audioCtx.state", this.audioCtx.state);
  ...
  FUPCMPlayer.prototype.getTimestamp = function(elapsedMs) {
    if (this.playingStartTime >= 0) {
      ...
      this.lastGotTimestamp += elapsedMs / 1e3;
      return ret;
    }
    return 0;
  };
  ...
  FUPCMPlayer.prototype.flush = function() {
    ...
    if (!this.flag_request_stop) {
      const nextTimeGap = (this.startTime - this.audioCtx.currentTime) * 1e3;
  ...
  // demo/src/websocket_connection.js
  var connect = function({ onData, onComplete }) {
